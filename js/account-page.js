(function () {
    ...
    myEdits.innerHTML = edits.map(function (e) {
      return (
        '<article class="tg-edit-card tg-edit-card--pending">' +
          ...
          '<p class="tg-edit-game">' + escapeHtml(e.game) + ' · submitted ' + new Date(e.createdAt).toLocaleString() + '</p>' +
          '<div class="tg-diff"><div><span>Proposed</span><pre>' + escapeHtml((e.proposedText || "").slice(0, 400)) + '</pre></div></div>' +
    ...
    myGuides.innerHTML = guides.map(function (g) {
      ...
      return (
        '<article class="tg-edit-card">' +
          '<header><strong>' + escapeHtml(g.title) + '</strong> <span class="tg-guide-status-badge">' + escapeHtml(g.type) + '</span></header>' +
          ...
          '<div class="tg-edit-actions"><a href="' + viewUrl + '" class="tg-btn tg-btn--ghost">' + (g.status === "published" ? "View" : "Edit") + '</a></div>' +
        '</article>'
      );
    }).join("");
  }
  ...
  function refresh() {
    TriumphCommunity.refreshSession().then(function (session) {
      if (!session) {
        guest.hidden = false;
        user.hidden = true;
        return;
      ...
      var avatarEl = document.getElementById("tg-account-avatar");
      avatarEl.src = TriumphCommunity.portraitForSlug(session.avatarSlug);
      avatarEl.alt = session.username + " avatar";
      adminLink.hidden = !TriumphCommunity.isAdmin();

      Promise.all([
        TriumphEdits.getPendingEdits(),
        TUG.getGuidesByAuthor(session.userId)
      ]).then(function (results) {
        renderMyEdits(results[0]);
        renderMyGuides(results[1]);
      });
    });
  }
  ...
  document.getElementById("tg-logout-account").addEventListener("click", function () {
    TriumphCommunity.logout().then(refresh);
  });
